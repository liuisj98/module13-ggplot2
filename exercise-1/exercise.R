...
ggplot(data = diamonds.sample) +
  ...
  # Draw a scatter plot (with point geometry) with for the `diamonds.sample` set,
  # with the `carat` mapped to the x-position and `price` mapped to the y-position.
  geom_point(aes(x = carat, y = cut, color = price, size = price))
...
ggplot(diamonds.sample) +
  geom_line(aes(x = carat, y = price, color = carat)) +
  geom_smooth(aes(x = carat, y = price, color = carat), se = FALSE)
...
ggplot(diamonds.sample) +
  geom_point(aes(x = carat, y = price, color = diamond)) +
